...
public class Ex05 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//이렇게 요청을 처리하기 위해서 함께 전달되는 정보를 파라미터라고한다.
		//파라미터의 자료형은 기본적으로 String이거나 String[] 이다.
		//클라이언트에서 요청할때 사용하는 메서드는 get방식, post방식 이다.
		//get 방식: 주소창에 정보가 보인다.(정보를 헤더에 보낸다.) (주소 url ~~~~ ?name=value&name=value~~~)
		//post 방식: 주소창에 정보가 안보인다.(정보를 바디에 보낸다.)
		...
		//만약에 s1 이라는 이름으로 숫자, s2라는 이름으로 숫자,op라는 이름으로  +가 넘어온다고 가정하자.
		...
		switch (op) {
		case "+": result = su1 + su2; break; //+는 %2B
