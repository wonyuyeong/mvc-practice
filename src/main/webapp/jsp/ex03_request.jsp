...
<body>
	<%--
		request(요청): 클라이언트가 요청한 정보를 담고 있음(파라미터도 갖고있음)
...
2. 파라미터의 정보 제공
	...
	**다른 주요 저장 메서드
		-forward	: request.getrequestDispacher("이동할주소").forward(request,response);
		-속성을 이용한 데이터 저장: request.setAttribute("이름",데이터);
		-속성에 저장된 데이터 호출: request.setAttribute("이름") 또는 ${이름} => EL;
