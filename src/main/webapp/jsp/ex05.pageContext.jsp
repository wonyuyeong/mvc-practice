...
<body>
	...
	JSP 페이지에서 1:1로 연결된 객체로 JSP페이지당 하나의 pageContext 객체가 생성
	같은 페이지에서 서로 값을 공유할 수 있으며, 다른 기본 내장객체를 구하거나, 페이지의 흐름을 제어한다.
	
	1.데이터 저장: pageContext,setAttribute("이름",객체);
	   데이터 호출: pageContext,getAttribute("이름"); => EL 사용 가능 ${이름}
	   생명 주기: 해당 페이지를 벗어나면 저장 정보도 사라진다.
	   
	**속성을 이용해서 데이터를 저장 및 호출하는 내장 객체들(생명주기는 반대)
	...
	application.setAttribute("name","마이콜"); //모든 브라우저를 다 끄고 다시 실행해야 나온다
	 %>
